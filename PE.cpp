...
PE::PE(size_t app_type)
{
	...
	*/
	...
	{ 
			...
			0x01, 0x4C, 0xCD, 0x21, 0x54, 0x68, 0x69, 0x73, 0x02, 0x70, 
			...
			0x64, 0x65, 0x2E, 0x0D, 0x0D, 0x0A, 0x24, 0x00, 0x00, 0x00, 
			0x00, 0x00, 0x00, 0x00 
	};
	...
	*/
	...
	// Fixed value (always?)
	this->ntHeader.FileHeader.SizeOfOptionalHeader = sizeof(IMAGE_OPTIONAL_HEADER);
	
	// Some standard flags
	this->ntHeader.FileHeader.Characteristics = IMAGE_FILE_RELOCS_STRIPPED | 
...
size_t 
PE::align_memory(size_t position)
{
	if(position == 0)
...
void 
PE::add_section(std::string sectionName, BYTE* data, size_t dataSize)
{
	...
	else
	{
		...
		this->sections[this->sections.size()-1]->header.VirtualAddress + 0x1000;
...
void
PE::fill_section_specific(size_t section_id)
{
	...
	{
		this->ntHeader.OptionalHeader.SizeOfUninitializedData = this->align_file(size_raw);
...
void 
PE::dump(std::string name)
{
	this->filename = name;
	this->ntHeader.OptionalHeader.SizeOfHeaders = this->align_file(this->ntHeader.OptionalHeader.SizeOfHeaders);
	...
	fwrite( (void*)&this->dosHeaderInfo.dosHeader, 1, sizeof(this->dosHeaderInfo.dosHeader), f);
	
	// Insert custom DOS program
	fwrite( (void*)this->dosHeaderInfo.code, 1, 
	this->dosHeaderInfo.code_size, f);
	...
	for(size_t i=0; i<this->ntHeader.FileHeader.NumberOfSections; i++)
		fwrite( (void*)&this->sections[i]->header, 1, sizeof(IMAGE_SECTION_HEADER), f);
	...
	size_t offset = dosHeaderInfo.dosHeader.e_lfanew + sizeof(IMAGE_NT_HEADERS) - (32 + sizeof(this->ntHeader.OptionalHeader.DataDirectory) );
...
void 
PE::import(std::string libname, std::string funcname)
{
	size_t rva = 0x1000;
	size_t vsize = 0x100;
	...
	if(this->sections.size() )
	{
		...
		if(import_section == 0xffff)
		{
			...
			+ this->align_memory(this->sections[this->sections.size()-1]->header.Misc.VirtualSize);
	...
	if(!this->imports.size() )
	{
		...
		if(section_num)
			importSec->header.PointerToRawData = this->sections[this->sections.size()-1]->header.PointerToRawData + this->sections[this->sections.size()-1]->header.SizeOfRawData;
		else
			importSec->header.PointerToRawData = 0x200;
		importSec->header.Characteristics = IMAGE_SCN_CNT_INITIALIZED_DATA |
		IMAGE_SCN_MEM_READ | IMAGE_SCN_ALIGN_4BYTES | IMAGE_SCN_MEM_WRITE;
		...
		this->ntHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress = rva;
	...
	if(!imported) // libraries
	{
		...
		memset(&tmp_import->importDirectoryTable, 0, sizeof(IMAGE_IMPORT_DESCRIPTOR) );
		...
		memset(&end_import->importDirectoryTable, 0, sizeof(IMAGE_IMPORT_DESCRIPTOR) );
...
void 
PE::alloc_section(std::string section_name, size_t section_size)
{
	this->ntHeader.FileHeader.NumberOfSections++;
	Section* sec = new Section;
	memset(&sec->header, 0, sizeof(IMAGE_SECTION_HEADER) );
	...
	{
		sec->header.VirtualAddress = this->sections[this->sections.size()-1]->header.VirtualAddress + 0x1000;
		sec->header.PointerToRawData = this->sections[this->sections.size()-1]->header.PointerToRawData + this->sections[this->sections.size()-1]->header.SizeOfRawData;
	}
...
size_t 
PE::write_section(std::string name, BYTE* data, size_t data_size)
{
...
size_t 
PE::flush_imports(size_t address)
{
	...
	if(this->imports.size() > 0)
	{
		...
		for(size_t i=0; i<this->imports.size()-1; i++)
		{
			...
			for(size_t j=0; j<this->imports[i]->importNameTable.size()-1; j++)
			{
				functions_length += sizeof(WORD) + strlen(this->imports[i]->importNameTable[j]->funcName)+1;
				functions++;
			}
		}
		
		// Import Name Table rva
		const size_t importNameTable = rva + this->imports.size() 
									 * sizeof(IMAGE_IMPORT_DESCRIPTOR);

		// dll name rva
		const size_t nameRVA = rva + this->imports.size() 
							 * sizeof(IMAGE_IMPORT_DESCRIPTOR)
							 + 2 * functions * sizeof(DWORD)
							 + functions_length;
		
		// IAT rva
		const size_t IAT = rva + this->imports.size()
						 * sizeof(IMAGE_IMPORT_DESCRIPTOR)
		...
		for(size_t i=0; i<this->imports.size()-1; i++)
		{
			this->imports[i]->importDirectoryTable.OriginalFirstThunk = importNameTable_var;
			importNameTable_var += this->imports[i]->importNameTable.size() 
								* sizeof(DWORD);
			...
			IAT_var += this->imports[i]->importNameTable.size() 
					* sizeof(DWORD);
			
		}
		
		// Fill directories
		this->ntHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].VirtualAddress = IAT;
		// at this place, IAT_var point at the end of IAT
		this->ntHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].Size = IAT_var - IAT;
		
		// alloc memory and map values into section->data
		size_t complete_size = this->imports.size() 
							 * sizeof(IMAGE_IMPORT_DESCRIPTOR) 
							 + 2 * functions * sizeof(DWORD)
							 + functions_length + libraries_length;
		
		// fill fields that required complete import size
		this->sections[import_section]->header.SizeOfRawData = this->align_file(complete_size);
		this->sections[import_section]->header.Misc.VirtualSize = complete_size;
		this->ntHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size = complete_size;
		
		this->sections[import_section]->data = new BYTE [this->align_file(complete_size) ];
		memset(this->sections[import_section]->data, 0, this->align_file(complete_size) );
		
		// copy all IDT entries
		size_t ptr = 0;
		for(size_t i=0; i<this->imports.size(); i++)
		{
			memcpy(this->sections[import_section]->data+ptr, (void*)&this->imports[i]->importDirectoryTable, sizeof(IMAGE_IMPORT_DESCRIPTOR) );
			ptr += sizeof(IMAGE_IMPORT_DESCRIPTOR);
		}
		
		// rva of first function name
		const size_t func_names_offset = rva + this->imports.size() 
							* sizeof(IMAGE_IMPORT_DESCRIPTOR) 
							+ 2 * functions * sizeof(DWORD);
		...
		for(size_t a=0; a<2; a++) // 2 means INT and IAT
		{
			...
			for(size_t i=0; i<this->imports.size()-1; i++)
			{
				for(size_t j=0; j<this->imports[i]->importNameTable.size()-1; j++)
				{
					...
					names_offset_var += sizeof(WORD) + strlen(this->imports[i]->importNameTable[j]->funcName)+1;
					
					memcpy(this->sections[import_section]->data+ptr, (void*)&this->imports[i]->importNameTable[j]->entry, sizeof(DWORD) );
					ptr += sizeof(DWORD);
				}
				// add blank entry at the end of every dll's INT
				memcpy(this->sections[import_section]->data+ptr, (void*)&this->imports[i]->importNameTable[this->imports[i]->importNameTable.size()-1]->entry, sizeof(DWORD) );
				ptr += sizeof(DWORD);
			}
		}
		...
		for(size_t i=0; i<this->imports.size()-1; i++)
		{
			for(size_t j=0; j<this->imports[i]->importNameTable.size()-1; j++)
			{
				memcpy(this->sections[import_section]->data+ptr, (void*)&this->imports[i]->importNameTable[j]->hint, sizeof(WORD) );
				ptr += sizeof(WORD);
				
				memcpy(this->sections[import_section]->data+ptr, this->imports[i]->importNameTable[j]->funcName, strlen(this->imports[i]->importNameTable[j]->funcName)+1);
		...
		for(size_t i=0; i<this->imports.size(); i++)
		{
			memcpy(this->sections[import_section]->data+ptr, this->imports[i]->libname, strlen(this->imports[i]->libname)+1);
...
size_t 
PE::get_iat_rv(std::string library, std::string funcname) const
{
	size_t lib_index = 0;
	...
	for(size_t i=0; i<this->imports[lib_index]->importNameTable.size(); i++)
		if(strcmp(this->imports[lib_index]->importNameTable[i]->funcName, funcname.c_str() ) == 0)
		{
			address = this->imports[lib_index]->importDirectoryTable.FirstThunk + i * sizeof(DWORD);
			break;
		}
...
PE::resize_section(std::string section_name, size_t new_size)
{
	...
	this->ntHeader.OptionalHeader.SizeOfImage -= this->align_memory(this->sections[section_id]->header.SizeOfRawData);
	this->ntHeader.OptionalHeader.SizeOfImage += this->align_memory(new_size);
	this->sections[section_id]->header.SizeOfRawData = new_size;
	
	this->sections[section_id]->header.Misc.VirtualSize = new_size;
	if(section_name == ".text")
		this->ntHeader.OptionalHeader.SizeOfCode = this->align_memory(new_size);
	
	for(size_t i=section_id; i<this->sections.size()-1; i++)
	{
		size_t pointer_to_data = this->sections[i]->header.PointerToRawData + this->sections[i]->header.SizeOfRawData;
		size_t rva = this->align_memory(this->sections[i]->header.VirtualAddress + this->sections[i]->header.Misc.VirtualSize);
		
		this->sections[i+1]->header.PointerToRawData = pointer_to_data;
		this->sections[i+1]->header.VirtualAddress = rva;
		if(strcmp( (char*)this->sections[i+1]->header.Name, ".idata") == 0)
		{
			this->ntHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress = rva;
			this->flush_imports(rva);
		}
	}
}
*/
...
WORD 
PE::RVAtoSection(DWORD rva)
{
	size_t i;
	for(i = 0; i < this->sections.size(); i++)
	{
		if(this->sections[i]->header.VirtualAddress <= rva &&
		this->align_memory(this->sections[i]->header.SizeOfRawData) + this->sections[i]->header.VirtualAddress > rva)
			return i;
	}
...
WORD 
PE::get_section_by_name(std::string section_name) const
{
	for(size_t i=0; i<this->sections.size(); i++)
	{
