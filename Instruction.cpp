...
uint 
Instruction::get(uchar *buffer, Instruction::Opcodes operation, 
			uint argument1, uint argument2, uint argument3, uint argument4)
{
	uint tmp = 0;
	...
	switch(operation)
	{
		...
		case Instruction::MOV_REG:
		{
			...
			uchar regs, sib;
			memcpy(buffer, (void*)&tmp, sizeof(uchar) );
			if(argument1 == Registers::ESP_SIB && argument3 != Mode::REG)
			{
				// argument1 == SIB
				// argument2 == source1
				// argument3 == source2
			...
			else if(argument1 == Registers::EBP_DISP && argument4 != 0)
			{
				// argument1 == DISP
				// argument2 == mode
		...
		case Instruction::CALL:
		{
			...
			if(argument1 == Registers::EBP_DISP && argument3 != Mode::REG)
			{
				instruction_size += sizeof(uint);
		...
		case Instruction::SUB_IMM:
		{
			...
			uchar reg, sib;
			memcpy(buffer, (void*)&tmp, sizeof(uchar) );
			if(argument1 == Registers::ESP_SIB && argument4 != 0)
			{
				// argument1 == SIB
				// argument2 == part1
				// argument3 == part2
...
uchar 
Instruction::modrm(uchar mode, uchar regop, uchar rm)
{
	return ( (mode << 6) | (regop << 3) | (rm << 0) );
}

uchar 
Instruction::sib(uchar scale, uchar index, uchar base)
{
